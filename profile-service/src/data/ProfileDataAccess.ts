...
export class ProfileDataAccess {
  ...
  async getGameRecord(
    ...
    console.log(`Looking for game record: userId=${userId}, gameId=${gameId}`);
  ...
  async addGameToAlbum(
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        console.log(`Game ${actualGameId} already in album ${albumId}`);
  ...
  async removeGameFromAlbum(
    ...
    try {
      ...
      console.log(
        success ? `Game removed from album` : `Failed to remove game from album`
      );
      return success;
    } catch (error) {
