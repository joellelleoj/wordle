...
class ProfileService {
  ...
  private async makeRequest<T>(
    ...
    return result as T; // Explicit type assertion to fix TypeScript error
  }
  ...
  // Get game history with pagination and better error handling
  async getGameHistory(limit = 20, offset = 0): Promise<GameRecord[]> {
    try {
      ...
      // Check authentication first
      if (!authService.isAuthenticated()) {
        logger.warn("Not authenticated - returning empty game history");
        return [];
      }
  ...
  // Record a completed game with better error handling
  async recordGame(gameData: Omit<GameRecord, "id">): Promise<void> {
    try {
      ...
      // Check authentication first
      if (!authService.isAuthenticated()) {
        logger.warn("Not authenticated - skipping game recording");
        return;
      }
      ...
      // Don't throw error - game recording failure shouldn't break the game
    }
  }

  // Album management with better error handling
  async getUserAlbums(): Promise<GameAlbum[]> {
    try {
      ...
      // Check authentication first
      if (!authService.isAuthenticated()) {
        logger.warn("Not authenticated - returning empty albums");
        return [];
      }
